...
export class Config {
    ...
    @IsNumber()
    @IsNotEmpty()
    maxCrawlDataTime: number

    @IsNumber()
    @IsNotEmpty()
    crawlDataSportDay: number
    ...
    constructor() {
        ...
        this.chunkSize = parseInt(env.CHUNK_SIZE)
        this.concurrency = parseInt(env.CONCURRENCY)
        this.crawlDataSportDay = parseInt(env.CRAWL_DATA_SPORT_DAY)
        this.maxCrawlDataTime = parseInt(env.MAX_CRAWL_DATA_TIME)
    }
...
export const chunkSize = config.chunkSize
//bien nao xai nhieu cho export o day luon
